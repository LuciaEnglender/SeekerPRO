...
router.post("/", async (req, res) => {
  try {
    const { language, seniority, skill, technology } = req.body;
      ...
      if (language) {
        if (typeof language === "string") {
          for (var j = 0; j < totalVacancies[i].languages.length; j++) {
            ...
            if (
              totalVacancies[i].languages[j]._previousDataValues.name ===
              language
            ) {
      ...
      if (seniority) {
        if (typeof seniority === "string") {
          for (var j = 0; j < totalVacancies[i].seniorities.length; j++) {
            //res.status(200).json(totalVacancies)//
            if (
              totalVacancies[i].seniorities[j]._previousDataValues.name ===
              seniority
            ) {
              arrVacancy.push(totalVacancies[i]);
              valor = true;
      ...
      if (skill) {
        if (typeof skill === "string") {
          for (var j = 0; j < totalVacancies[i].skills.length; j++) {
            //res.status(200).json(totalVacancies)//
            if (
              totalVacancies[i].skills[j]._previousDataValues.name === skill
            ) {
              arrVacancy.push(totalVacancies[i]);
              valor = true;
              break;
          ...
          for (var j = 0; j < totalVacancies[i].skills.length; j++) {
            ...
            for (var k = 0; k < skill.length; k++) {
              if (
                totalVacancies[i].skills[j]._previousDataValues.name ===
                skill[k]
              ) {
                arrVacancy.push(totalVacancies[i]);
                valor2 = true;
      ...
      if (technology) {
        if (typeof technology === "string") {
          for (var j = 0; j < totalVacancies[i].technologies.length; j++) {
            //res.status(200).json(totalVacancies)//
            if (
              totalVacancies[i].technologies[j]._previousDataValues.name ===
              technology
            ) {
              arrVacancy.push(totalVacancies[i]);
              valor = true;
